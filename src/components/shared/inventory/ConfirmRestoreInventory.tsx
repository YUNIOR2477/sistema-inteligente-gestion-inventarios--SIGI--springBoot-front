...
export const ConfirmRestoreInventory = ({
  inventoryId,
  isMobile=false,
  onDone,
}: RestoreProps) => {
  const { handleRestoreInventory, isLoading } = useInventoryStore();
  ...
  const handleConfirmRestore = async () => {
    try {
      const response = await handleRestoreInventory(inventoryId);
      if (response) {
        ToastMessage({
          type: "success",
          title: response.message,
          description: `Movement with ID ${inventoryId} has been restored successfully.`,
        });
        onDone();
        setOpen(false);
      }

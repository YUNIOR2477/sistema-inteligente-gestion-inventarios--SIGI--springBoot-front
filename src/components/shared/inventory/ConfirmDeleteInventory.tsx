...
export const ConfirmDeleteInventory = ({
  inventoryId,
  isMobile = false,
  onDone,
}: DeleteProps) => {
  const { handleDeleteInventory, isLoading } = useInventoryStore();
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const response = await handleDeleteInventory(inventoryId);
      ToastMessage({
        type: "success",
        title: response?.message,
        description: `Movement with ID ${inventoryId} has been removed successfully.`,
      });
      onDone();
      setOpen(false);
    } catch (err: unknown) {

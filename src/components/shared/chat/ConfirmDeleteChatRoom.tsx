...
export const ConfirmDeleteChatRoom = ({
  chatRoomId,
  isMobile = false,
  onDone,
}: DeleteProps) => {
  const { handleDeactivateRoomById, isLoading } = useChatRoomStore();
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const response = await handleDeactivateRoomById(chatRoomId);
      ToastMessage({
        type: "success",
        title: response?.message || "Chat room deleted",
        description: `Chat room with ID ${chatRoomId} has been removed successfully.`,
      });
      onDone();
      setOpen(false);
    } catch (err: unknown) {

...
export const ConfirmDeleteInvoice = ({ invoiceId, isMobile, onDone }: DeleteProps) => {
  const { handleDeleteInvoice, isLoading } = useInvoiceStore();
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const response = await handleDeleteInvoice(invoiceId);
      ToastMessage({
        type: "success",
        title: response?.message || "Invoice deleted",
        description: `Invoice with ID ${invoiceId} has been removed successfully.`,
      }); 
      onDone();
      setOpen(false);
    } catch (err: unknown) {

...
export const ConfirmRestoreClient = ({
  clientId,
  isMobile=false,
  onDone,
}: RestoreProps) => {
  const { handleRestoreClient, isLoading } = useClientStore();
  ...
  const handleConfirmRestore = async () => {
    try {
      const response = await handleRestoreClient(clientId);
      if (response) {
        ToastMessage({
          type: "success",
          title: response.message || "Client restored",
          description: `Client with ID ${clientId} has been restored successfully.`,
        });
        onDone();
        setOpen(false);
      }

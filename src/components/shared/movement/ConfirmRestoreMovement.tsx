...
export const ConfirmRestoreMovement = ({
  movementId,
  isMobile,
  onDone,
}: RestoreProps) => {
  const { handleRestoreMovement, isLoading } = useMovementStore();
  ...
  const handleConfirmRestore = async () => {
    try {
      const response = await handleRestoreMovement(movementId);
      if (response) {
        ToastMessage({
          type: "success",
          title: response.message,
          description: `Movement with ID ${movementId} has been restored successfully.`,
        });
        onDone();
        setOpen(false);
      }

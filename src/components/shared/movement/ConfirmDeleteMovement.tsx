...
export const ConfirmDeleteMovement = ({ movementId,isMobile, onDone }: DeleteProps) => {
  const { handleDeleteMovement, isLoading } = useMovementStore();
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const response = await handleDeleteMovement(movementId);
      ToastMessage({
        type: "success",
        title: response?.message,
        description: `Movement with ID ${movementId} has been removed successfully.`,
      });
      onDone();
      setOpen(false);
    } catch (err: unknown) {

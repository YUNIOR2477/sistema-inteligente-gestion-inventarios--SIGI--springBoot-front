...
export const ConfirmRestoreDispatcher = ({
  dispatcherId,
  isMobile = false,
  onDone,
}: RestoreProps) => {
  const { handleRestoreDispatcher, isLoading } = useDispatcherStore();
  ...
  const handleConfirmRestore = async () => {
    try {
      const response = await handleRestoreDispatcher(dispatcherId);
      if (response) {
        ToastMessage({
          type: "success",
          title: response.message || "Dispatcher restored",
          description: `Dispatcher with ID ${dispatcherId} has been restored successfully.`,
        });
        onDone();
        setOpen(false);
      }
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription>
            Are you sure you want to restore this Dispatcher?
          </DialogDescription>
          <Separator />
        </DialogHeader>

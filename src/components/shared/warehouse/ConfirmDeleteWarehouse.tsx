...
export const ConfirmDeleteWarehouse = ({ warehouseId, isMobile,onDone }: DeleteProps) => {
  const { handleDeleteWarehouse, isLoading } = useWarehouseStore();
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const response = await handleDeleteWarehouse(warehouseId);
      ToastMessage({
        type: "success",
        title: response?.message || "Warehouse deleted",
        description: `Warehouse with ID ${warehouseId} has been removed successfully.`,
      }); 
      onDone();
      setOpen(false);
    } catch (err: unknown) {

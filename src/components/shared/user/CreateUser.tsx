...
export const CreateUser = () => {
  const { handleCreateUser, isLoading } = useUserStore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response) {
        ...
        localStorage.setItem("createUserForm", "");
        ToastMessage({
          type: "success",
          title: response.message,
          description: `The new user has been saved in the database, and its ID: ${response.data.id} has been copied to the clipboard.`,
        });
    ...
    <form onSubmit={handleSubmit}>
      <SheetHeader>
        ...
        <SheetDescription className="justify-center flex">
          Fill in the User details below. Click save when you are done.
        </SheetDescription>
      </SheetHeader>
      <Separator />
      ...
      <SheetFooter>
        ...
        >
          <FilePen /> {isLoading ? "Saving..." : "Save User"}
        </Button>
        <SheetClose asChild>
          <Button
            variant={"ghost"}

...
export const ConfirmDeleteUser = ({ userId,isMobile, onDone }: DeleteProps) => {
  const { handleDeactivateUser, isLoading } = useUserStore();
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const response = await handleDeactivateUser(userId);
      ToastMessage({
        type: "success",
        title: response?.message || "User deleted",
        description: `User with ID ${userId} has been removed successfully.`,
      }); 
      onDone();
      setOpen(false);
    } catch (err: unknown) {

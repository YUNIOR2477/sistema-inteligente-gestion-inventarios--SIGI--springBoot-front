...
export const ConfirmRestoreUser = ({
  userId,
  onDone,isMobile
}: RestoreProps) => {
  const {
  ...
  const handleConfirmRestore = async () => {
    try {
      const response = await handleActivateUser(userId);
      if (response) {
        ToastMessage({
          type: "success",
          title: response.message || "User restored",
          description: `User with ID ${userId} has been restored successfully.`,
        });
        onDone();
        setOpen(false);
      }
